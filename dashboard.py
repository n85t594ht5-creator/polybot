...
<header>
  <div class="menu" id="menu"><button id="menuBtn">☰ Меню</button><div class="dd">
    <a href="#" data-view="dash" class="on">Дашборд</a><a href="backtest.html">Тестировщик</a><a href="#" data-view="settings">Настройки бота</a><a href="#" data-view="keys">Ключи</a><a href="#" data-view="power">Питание</a></div></div>
  <h1>POLYBOT</h1>
...
function showView(v){document.querySelectorAll('.view').forEach(x=>x.classList.toggle('on',x.id==='v_'+v));document.querySelectorAll('#menu .dd a').forEach(a=>a.classList.toggle('on',a.dataset.view===v));$('menu').classList.remove('open');
 if(v==='settings')loadSettings();if(v==='keys')loadKeys();if(v==='power')loadPower()}
// settings schema
const S=[
...
$('setSave').onclick=async()=>{if(!tok())return toast('Сначала добавь токен GitHub в «Ключи»');const v=readSettings();if(!v.ASSETS||!v.WINDOWS)return toast('Выбери хотя бы один актив и одно окно');
 ...
 $('setMsg').textContent='сохраняю…';try{for(const[k,val]of Object.entries(v))if(String(VARS[k]??DEF[k])!==String(val))await setVar(k,val);await restartBot();$('setMsg').textContent='сохранено, бот перезапускается';toast('Настройки сохранены')}catch(e){$('setMsg').textContent='ошибка: '+e.message}};
