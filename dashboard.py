...
def read_signals(n=2000):
    """Журнал qualifying signals. Числовые поля приводим к float, пустые оставляем None."""
    if not os.path.exists(SIGNALS_FILE):
        return []
    num = {"elapsed", "remaining_seconds", "reference_price", "current_price", "move_pct",
           "entry_price", "confidence", "required_move", "requested_shares", "filled_shares",
           "remaining_shares", "average_fill_price", "best_ask", "slippage", "hyp_cost",
           "hyp_shares", "hypothetical_pnl", "realized_pnl", "bankroll_at_signal", "daily_pnl",
           "exposure_at_signal", "consecutive_losses", "spread", "bid", "ask"}
    out = []
    with open(SIGNALS_FILE, encoding="utf-8", errors="ignore") as f:
        for r in csv.DictReader(f):
            row = {}
            for k, v in r.items():
                if k in num:
                    try: row[k] = float(v) if v not in ("", None) else None
...
.grid{display:grid;gap:12px;grid-template-columns:repeat(12,1fr)}.grid>*{min-width:0}
.card{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:14px 16px;min-width:0;overflow:hidden}
...
.fn .s{grid-template-columns:minmax(0,92px) minmax(0,1fr) minmax(0,auto);font-size:11px}.fn .s i{height:16px}
.tabs button{padding:6px 9px;font-size:11px}.tabs{gap:4px}
#stFilters button{padding:5px 8px}
.mc{padding:10px}.hyp{padding-left:6px}}
...
td{padding:7px 8px;border-bottom:1px solid rgba(36,50,72,.6);white-space:nowrap;max-width:320px;overflow:hidden;text-overflow:ellipsis}
tr:last-child td{border-bottom:0}
.tw{overflow:auto;max-height:420px}
.gate{display:grid;grid-template-columns:minmax(0,130px) minmax(0,1fr) minmax(0,auto);gap:8px;align-items:center;padding:7px 0;border-bottom:1px solid rgba(36,50,72,.6)}
.gate>*{min-width:0}
.gate:last-child{border-bottom:0}
.gate .n{font-size:12px;overflow:hidden;text-overflow:ellipsis}
.gate .v{font:12px var(--mono);color:var(--mut);text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.kv{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,auto);gap:4px 12px;font:12px var(--mono)}.kv span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.set{display:grid;grid-template-columns:minmax(0,1fr);gap:10px}.set .row{background:var(--panel2);border:1px solid var(--line);border-radius:10px;padding:12px 14px}
...
.feed{max-height:420px;overflow:auto}.ev{display:grid;grid-template-columns:26px minmax(0,1fr) minmax(0,auto);gap:10px;padding:9px 4px;border-bottom:1px solid rgba(36,50,72,.6);font-size:13px;align-items:center}
.ev .ic{font-size:15px;text-align:center}.ev .tm{font:11px var(--mono);color:var(--mut)}.ev>span:nth-child(2){overflow:hidden}.ev>span:last-child{white-space:nowrap}.ev b{font-weight:600}
.cal{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:4px}
.cal .d{min-width:0;min-height:46px;border-radius:6px;background:var(--panel2);border:1px solid var(--line);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;font:12px var(--mono);cursor:pointer;padding:4px 2px;overflow:hidden}
.cal .d small{font-size:9px;color:var(--mut);line-height:1;white-space:nowrap;overflow:hidden;max-width:100%}.cal .d.win{background:rgba(53,217,155,.18);border-color:var(--up)}.cal .d.loss{background:rgba(255,107,122,.18);border-color:var(--down)}.cal .d.sel{outline:2px solid var(--acc)}
.cal .hd{text-align:center;font:10px var(--mono);color:var(--mut);letter-spacing:.08em}
.hgrid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:3px}
.hgrid div{min-width:0;height:26px;border-radius:4px;background:var(--panel2);border:1px solid var(--line);font:10px var(--mono);display:flex;align-items:center;justify-content:center;color:var(--mut);overflow:hidden}
...
.book{font:11px var(--mono)}.book .r{display:grid;grid-template-columns:44px minmax(0,1fr) 58px;gap:6px;align-items:center;padding:1px 0}.book .r i{display:block;height:8px;background:rgba(138,180,255,.35);border-radius:2px}
.fn{display:grid;gap:6px}.fn .s{display:grid;grid-template-columns:minmax(0,124px) minmax(0,1fr) minmax(0,auto);gap:8px;align-items:center;font:12px var(--mono)}
.fn .s i{display:block;height:20px;border-radius:4px;background:linear-gradient(90deg,var(--acc),rgba(138,180,255,.25))}
.fn .s.drop i{background:linear-gradient(90deg,var(--warn),rgba(245,182,74,.2))}.fn .s.fill i{background:linear-gradient(90deg,var(--up),rgba(53,217,155,.25))}
.fn .s span:last-child{text-align:right;color:var(--mut);white-space:nowrap}
.hyp{border-left:2px solid var(--warn);padding-left:8px}
.mc{background:var(--surface-1,var(--panel2));border-radius:10px;padding:12px}.mc .lab{font-size:13px;color:var(--mut);margin:0 0 4px}
.brow{display:grid;grid-template-columns:minmax(0,96px) minmax(0,1fr) minmax(0,auto);gap:8px;align-items:center;padding:5px 0;font:12px var(--mono);border-bottom:1px solid rgba(36,50,72,.5)}
.brow>*{min-width:0}
...
.md table{width:100%;border-collapse:collapse;font:12px var(--mono);margin-top:4px;display:block;overflow-x:auto}.md td{padding:5px 8px;border-bottom:1px solid rgba(36,50,72,.6);white-space:nowrap}
...
<div class="grid">
  ...
  <div class="card c6"><h2>Воронка сигналов</h2><div id="funnel"></div><div class="sub" id="funnelSub"></div></div>
  <div class="card c6"><h2>5m против 15m · realized</h2><div id="perWindow"></div><div class="sub">Две отдельные стратегии внутри одной системы. Подробности — в Статистике и Исполнении.</div></div>
...
<div id="v_statistics" class="view">
 <div class="tabs" id="stMode" style="margin-bottom:8px"></div>
 <div class="tabs" id="stTabs" style="margin-bottom:8px"></div>
 <div class="tabs" id="stFilters" style="margin-bottom:12px"></div>
  <div class="grid">
   <div class="card c12" id="modeNote" style="display:none"></div>
   ...
   <div class="card c6"><h2>По силе движения</h2><div id="st_move"></div><div class="sub" id="st_moveNote"></div></div>
   <div class="card c6"><h2>5m против 15m</h2><div id="st_win"></div></div>
   <div class="card c6"><h2>По активам</h2><div id="st_asset"></div></div>
   <div class="card c12"><h2>Концентрация результата</h2><div id="st_conc"></div><div class="sub">Если один день или один срез даёт непропорционально много — это событие, а не стратегия.</div></div>
...
<div id="v_execution" class="view">
 <div class="sub" style="margin-bottom:10px">Отвечает на вопрос: проблема в стратегии или в исполнении? Слева — путь сигнала, справа — почему сигналы теряются.</div>
 <div class="grid">
  <div class="card c6"><h2>Воронка</h2><div id="ex_funnel"></div><div class="sub" id="ex_funnelSub"></div></div>
  <div class="card c6"><h2>Причины потерь</h2><div id="ex_reasons"></div><div class="sub">Риск-гейты и неисполненные ордера.</div></div>
  <div class="card c4"><h2>Execution rate</h2><div class="big" id="ex_rate">—</div><div class="sub" id="ex_rateSub"></div></div>
  <div class="card c4"><h2>Среднее проскальзывание</h2><div class="big" id="ex_slip">—</div><div class="sub">средняя цена исполнения минус лучшая цена</div></div>
  <div class="card c4"><h2>Качество исполнения</h2><div id="ex_quality"></div><div class="sub">GOOD — запас времени и объёма, MARGINAL — впритык, UNEXECUTABLE — исполнить было нельзя</div></div>
  <div class="card c12"><h2>5m против 15m</h2><div class="tw"><table><thead><tr><th>Окно</th><th>Сигналов</th><th>Прошли риск</th><th>Исполнено</th><th>Частично</th><th>Не исполнено</th><th>Execution rate</th><th>Ср. остаток времени</th><th>GOOD / MARGINAL / UNEXEC</th></tr></thead><tbody id="ex_win"></tbody></table></div></div>
  <div class="card c12"><h2>Последние сигналы</h2><div class="tw"><table><thead><tr><th>Время</th><th>Актив</th><th>Окно</th><th>Сторона</th><th>Вход</th><th>Статус</th><th>Риск-гейт</th><th>Ордер</th><th>Качество</th><th>Осталось, с</th></tr></thead><tbody id="ex_list"></tbody></table></div></div>
 </div>
</div>
...
function render(d){
  ...
  // воронка сигналов
  const sg=d.signals||[];
  const nSig=sg.length+(d.pending_signals||0), nRisk=sg.filter(s=>!s.risk_gate).length,
        nOrd=sg.filter(s=>s.signal_status&&s.signal_status.indexOf('ORDER_')===0||s.signal_status==='EXECUTED').length,
        nFill=sg.filter(s=>s.signal_status==='EXECUTED').length,
        nBlock=sg.filter(s=>s.signal_status==='BLOCKED_BY_RISK').length;
  const mx=Math.max(nSig,1);
  const step=(lbl,v,cl)=>`<div class="s ${cl||''}"><span>${lbl}</span><i style="width:${Math.max(2,v/mx*100).toFixed(0)}%"></i><span>${v}</span></div>`;
  $('funnel').innerHTML=`<div class="fn">${step('Сигналов',nSig)}${step('Прошли риск',nRisk)}${step('Ордеров',nOrd)}${step('Исполнено',nFill,'fill')}${nBlock?step('Заблокировано',nBlock,'drop'):''}</div>`;
  const gateCnt={};sg.forEach(s=>{if(s.risk_gate)gateCnt[s.risk_gate]=(gateCnt[s.risk_gate]||0)+1});
  $('funnelSub').innerHTML=nSig?`исполнение ${(nFill/Math.max(nSig,1)*100).toFixed(0)}%${Object.keys(gateCnt).length?' · блокировки: '+Object.entries(gateCnt).map(([k,v])=>k+' '+v).join(', '):''}${d.pending_signals?' · ждут резолва: '+d.pending_signals:''}`:'Сигналов пока нет — бот ждёт подходящее окно.';
  // 5m vs 15m
...
// ───── статистика ─────
let stMode='month', stDay=null, stView='realized', stF={window:'',asset:'',dir:'',bucket:''};
// REALIZED — реальные сделки; SIGNALS — все qualifying signals (гипотетический P&L)
function allTrades(){
 if(stView==='realized')return (last&&last.closed||[]).map(t=>({...t,_pnl:t.pnl,_when:t.opened,_win:t.won})).sort((a,b)=>Date.parse(a._when)-Date.parse(b._when));
 return ((last&&last.signals)||[]).filter(s=>s.resolution).map(s=>({
   opened:s.timestamp,_when:s.timestamp,asset:s.asset,side:s.direction,minutes:parseInt(s.window)||0,
   entry:s.entry_price,cost:s.hyp_cost,entry_bucket:s.entry_bucket,move_bucket:s.move_bucket,
   elapsed_bucket:s.elapsed_bucket,won:s.resolution==='WIN',_win:s.resolution==='WIN',
   pnl:s.hypothetical_pnl,_pnl:s.hypothetical_pnl,status:s.signal_status,gate:s.risk_gate
 })).sort((a,b)=>Date.parse(a._when)-Date.parse(b._when));
}
function applyF(l){return l.filter(t=>(!stF.window||String(t.minutes)+'m'===stF.window)&&(!stF.asset||t.asset===stF.asset)&&(!stF.dir||t.side===stF.dir)&&(!stF.bucket||t.entry_bucket===stF.bucket))}
...
function renderStats(){
 $('stMode').innerHTML=[['realized','REALIZED · реальные сделки'],['signals','SIGNALS · гипотетический P&L'],['comparison','СРАВНЕНИЕ']]
  .map(([k,n])=>`<button class="${k===stView?'on':''}" onclick="stView='${k}';renderStats()">${n}</button>`).join('');
 ...
 const sgAll=((last&&last.signals)||[]).filter(s=>s.resolution);
 const opts=(key,vals)=>vals.map(v=>`<button class="${stF[key]===v?'on':''}" onclick="stF.${key}=stF.${key}==='${v}'?'':'${v}';renderStats()">${v||'все'}</button>`).join('');
 $('stFilters').innerHTML='<span class="sub" style="align-self:center;margin-right:4px">фильтры:</span>'
  +opts('window',['5m','15m'])+opts('asset',['BTC','ETH','SOL','XRP'])+opts('dir',['UP','DOWN'])+opts('bucket',['0.50–0.55','0.55–0.60','0.60–0.62']);
 if(stView==='comparison'){renderComparison();return}
 $('modeNote').style.display=stView==='signals'?'':'none';
 if(stView==='signals')$('modeNote').innerHTML='<div class="warnbox" style="margin:0">Это <b>HYPOTHETICAL P&amp;L</b> по всем qualifying signals, включая заблокированные риск-гейтами и неисполненные ордера. Реальными деньгами это не подтверждено — переключись на REALIZED, чтобы увидеть фактический результат.</div>';
 const all=applyF(allTrades()),list=all.filter(inPeriod);
 ...
 const MB=['0.00–0.05%','0.05–0.10%','0.10–0.12%','0.12–0.15%','0.15–0.20%','0.20%+'];
 const mvg=grp(list,t=>t.move_bucket||'—');
 MB.forEach(k=>{if(!mvg[k])mvg[k]={n:0,w:0,pnl:0}});
 $('st_move').innerHTML=MB.map(k=>{const v=mvg[k],below=k==='0.00–0.05%'||k==='0.05–0.10%';
  if(!v.n)return `<div class="brow"><span class="k">${k}</span><span class="t"></span><span class="v" style="opacity:.6">${below?'отсекается фильтром':'нет сделок'}</span></div>`;
  const wr=v.w/v.n;return `<div class="brow"><span class="k">${k}</span><span class="t"><i style="width:${(wr*100).toFixed(0)}%;background:${v.pnl>=0?'var(--up)':'var(--down)'}"></i></span><span class="v">${(wr*100).toFixed(0)}% · ${v.n} · <span class="${cls(v.pnl)}">${sgn(v.pnl)}</span></span></div>`}).join('');
 const C3=(last&&last.config)||{};
 $('st_moveNote').innerHTML=`Движение ниже ${((C3.MIN_MOVE||0.001)*100).toFixed(2)}% бот не торгует, поэтому два верхних бакета здесь всегда пусты. Чтобы увидеть, что происходило бы с отсечёнными сигналами — <a href="backtest.html">Тестировщик → Move Analysis</a>.`;
 barList('st_win',grp(list,t=>(t.minutes||'?')+'m'));
 barList('st_asset',grp(list,t=>t.asset));
 // концентрация
 const byDay=grp(list,t=>t.opened.slice(0,10));
 const days=Object.entries(byDay).map(([k,v])=>[k,v.pnl]).sort((a,b)=>Math.abs(b[1])-Math.abs(a[1]));
 const totAbs=days.reduce((s,[,p])=>s+Math.abs(p),0)||1;
 const totPnl=list.reduce((s,t)=>s+t.pnl,0);
 const share=(arr)=>arr.reduce((s,[,p])=>s+Math.abs(p),0)/totAbs*100;
 const conc=[];
 if(days.length){conc.push(['Лучший день: '+days[0][0],share(days.slice(0,1))]);
  if(days.length>2)conc.push(['Топ-3 дня',share(days.slice(0,3))]);}
 const byA=grp(list,t=>t.asset),byW=grp(list,t=>(t.minutes||'?')+'m'),byB=grp(list,t=>t.entry_bucket||'—');
 const topOf=o=>{const e=Object.entries(o).sort((a,b)=>Math.abs(b[1].pnl)-Math.abs(a[1].pnl));return e.length?[e[0][0],Math.abs(e[0][1].pnl)/totAbs*100]:null};
 [['Топ актив',topOf(byA)],['Топ окно',topOf(byW)],['Топ бакет входа',topOf(byB)]].forEach(([lbl,v])=>{if(v)conc.push([lbl+': '+v[0],v[1]])});
 $('st_conc').innerHTML=days.length?conc.map(([k,p])=>`<div class="brow"><span class="k">${esc(k)}</span><span class="t"><i style="width:${Math.min(100,p).toFixed(0)}%;background:${p>60?'var(--down)':p>35?'var(--warn)':'var(--up)'}"></i></span><span class="v">${p.toFixed(0)}% результата</span></div>`).join('')
   +`<div class="sub" style="margin-top:6px">${days.length} дней со сделками · итог ${sgn(totPnl)} $${days.length&&share(days.slice(0,1))>60?' · <span class="neg">результат держится на одном дне</span>':''}</div>`
...
function renderComparison(){
 ...
 $('modeNote').innerHTML='<div class="warnbox" style="margin:0">Слева — <b>REALIZED</b>: деньги, которые бот действительно заработал или потерял. Справа — <b>HYPOTHETICAL</b>: что было бы, если бы исполнялись все qualifying signals, включая заблокированные риском. Это разные метрики, складывать их нельзя.</div>';
 const sv=stView; stView='realized'; const real=applyF(allTrades()).filter(inPeriod);
 stView='signals'; const sig=applyF(allTrades()).filter(inPeriod); stView=sv;
 const agg=l=>{const w=l.filter(t=>t.won),gp=w.reduce((s,t)=>s+t.pnl,0),gl=-l.filter(t=>!t.won).reduce((s,t)=>s+t.pnl,0);
  return{n:l.length,w:w.length,wr:l.length?w.length/l.length:0,pf:gl?gp/gl:(gp?99:0),pnl:l.reduce((s,t)=>s+t.pnl,0)}};
 const a=agg(real),b=agg(sig);
 const card=(t,m,cls2)=>`<div class="mc"><p class="lab">${t}</p><div class="big m ${cls(m.pnl)}">${sgn(m.pnl)} $</div><div class="sub">${m.n} шт · winrate ${(m.wr*100).toFixed(0)}% · PF ${m.pf>=99?'∞':fmt(m.pf)}</div></div>`;
 $('st_pnl').textContent=sgn(a.pnl)+' $';$('st_pnl').className='big '+cls(a.pnl);$('st_sub').textContent=`REALIZED · ${a.n} сделок`;
 $('st_wr').textContent=a.n?(a.wr*100).toFixed(0)+'%':'—';$('st_wl').textContent=`${a.w} побед / ${a.n-a.w} убытков`;
 $('st_pf').textContent=a.pf>=99?'∞':fmt(a.pf);$('st_pf').className='big '+(a.pf>=1.3?'pos':a.pf>=1?'warn':'neg');
 $('st_pfs').innerHTML=`<span class="hyp">HYPOTHETICAL: ${b.n} сигналов · winrate ${(b.wr*100).toFixed(0)}% · PF ${b.pf>=99?'∞':fmt(b.pf)} · <span class="${cls(b.pnl)}">${sgn(b.pnl)} $</span></span>`;
 $('calTitle').textContent='Realized против Hypothetical по дням';
 const dr=grp(real,t=>t.opened.slice(0,10)),ds=grp(sig,t=>t.opened.slice(0,10));
 const keys=[...new Set([...Object.keys(dr),...Object.keys(ds)])].sort();
 $('calBox').innerHTML=keys.length?`<div class="tw" style="max-height:280px"><table><thead><tr><th>День</th><th>Realized сделок</th><th>Realized P&L</th><th>Сигналов</th><th>Hypothetical P&L</th></tr></thead><tbody>`+
  keys.map(k=>`<tr><td>${k}</td><td>${dr[k]?dr[k].n:0}</td><td class="${cls(dr[k]?dr[k].pnl:0)}">${dr[k]?sgn(dr[k].pnl):'—'}</td><td>${ds[k]?ds[k].n:0}</td><td class="${cls(ds[k]?ds[k].pnl:0)}">${ds[k]?sgn(ds[k].pnl):'—'}</td></tr>`).join('')+'</tbody></table></div>':'<div class="empty">нет данных</div>';
 $('calHint').textContent='Расхождение показывает, сколько мы теряем на риск-гейтах и исполнении.';
 const pair=(id,fn)=>{const A=grp(real,fn),B=grp(sig,fn);const ks=[...new Set([...Object.keys(A),...Object.keys(B)])].sort();
  $(id).innerHTML=ks.length?`<table><thead><tr><th>Срез</th><th>Real</th><th>Real P&L</th><th>Сигн.</th><th>Hyp P&L</th></tr></thead><tbody>`+
   ks.map(k=>`<tr><td>${esc(k)}</td><td>${A[k]?A[k].n:0}</td><td class="${cls(A[k]?A[k].pnl:0)}">${A[k]?sgn(A[k].pnl):'—'}</td><td>${B[k]?B[k].n:0}</td><td class="${cls(B[k]?B[k].pnl:0)}">${B[k]?sgn(B[k].pnl):'—'}</td></tr>`).join('')+'</tbody></table>':'<div class="empty">нет данных</div>'};
 pair('st_win',t=>(t.minutes||'?')+'m'); pair('st_asset',t=>t.asset);
 pair('st_entry',t=>t.entry_bucket||'—'); pair('st_move',t=>t.move_bucket||'—');
 $('st_hours').innerHTML='';$('st_conc').innerHTML='<div class="sub">Переключись на REALIZED или SIGNALS, чтобы увидеть концентрацию.</div>';
 $('st_dd').innerHTML='';$('st_dds').textContent='';
 $('st_trades').innerHTML=sig.slice().reverse().slice(0,80).map(t=>`<tr><td>${t.opened.slice(5,16).replace('T',' ')}</td><td>${esc(t.asset)}</td><td>${t.minutes}м</td><td class="${t.side==='UP'?'pos':'neg'}">${esc(t.side)}</td><td>${fmt(t.entry)}</td><td>${fmt(t.cost)}</td><td class="${t.won?'pos':'neg'}">${t.won?'WIN':'LOSS'}</td><td class="${cls(t.pnl)}">${sgn(t.pnl)}</td></tr>`).join('')||'<tr><td colspan="8" class="empty">нет сигналов</td></tr>';
}
// ───── журнал сигналов ─────
let sgFilter='all';
...
// ───── исполнение ─────
function sigList(){return (last&&last.signals)||[]}
function renderExec(){const sg=sigList();
 const nSig=sg.length,nRisk=sg.filter(s=>!s.risk_gate).length,
  nFill=sg.filter(s=>s.signal_status==='EXECUTED').length,
  nPart=sg.filter(s=>s.order_status==='PARTIAL').length,
  nUnf=sg.filter(s=>s.signal_status==='ORDER_UNFILLED').length,
  nCan=sg.filter(s=>s.signal_status==='ORDER_CANCELLED').length;
 const mx=Math.max(nSig,1),step=(l,v,c)=>`<div class="s ${c||''}"><span>${l}</span><i style="width:${Math.max(2,v/mx*100).toFixed(0)}%"></i><span>${v}</span></div>`;
 $('ex_funnel').innerHTML=`<div class="fn">${step('Сигналов',nSig)}${step('Прошли риск',nRisk)}${step('Ордеров',nRisk)}${step('Исполнено',nFill,'fill')}</div>`;
 $('ex_funnelSub').textContent=nSig?`из ${nSig} сигналов исполнено ${nFill}`:'нет данных';
 const rs={};sg.forEach(s=>{if(s.risk_gate)rs[s.risk_gate]=(rs[s.risk_gate]||0)+1});
 if(nUnf)rs['ОРДЕР НЕ ИСПОЛНЕН']=nUnf; if(nCan)rs['ОРДЕР СНЯТ']=nCan; if(nPart)rs['ЧАСТИЧНОЕ ИСПОЛНЕНИЕ']=nPart;
 const tot=Object.values(rs).reduce((a,b)=>a+b,0)||1;
 $('ex_reasons').innerHTML=Object.entries(rs).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div class="brow"><span class="k">${esc(k)}</span><span class="t"><i style="width:${(v/tot*100).toFixed(0)}%;background:var(--warn)"></i></span><span class="v">${v}</span></div>`).join('')||'<div class="empty">Потерь не было.</div>';
 $('ex_rate').textContent=nSig?(nFill/nSig*100).toFixed(0)+'%':'—';
 $('ex_rate').className='big '+(nSig?(nFill/nSig>=.7?'pos':nFill/nSig>=.4?'warn':'neg'):'');
 $('ex_rateSub').textContent=`${nFill} исполнено из ${nSig} сигналов`;
 const sl=sg.filter(s=>s.slippage!=null);
 $('ex_slip').textContent=sl.length?(sl.reduce((a,s)=>a+s.slippage,0)/sl.length).toFixed(4):'—';
 const q={};sg.forEach(s=>{const k=s.execution_quality||'—';q[k]=(q[k]||0)+1});
 const qt=Object.values(q).reduce((a,b)=>a+b,0)||1;
 $('ex_quality').innerHTML=Object.entries(q).sort().map(([k,v])=>`<div class="brow"><span class="k">${esc(k)}</span><span class="t"><i style="width:${(v/qt*100).toFixed(0)}%;background:${k==='GOOD'?'var(--up)':k==='MARGINAL'?'var(--warn)':'var(--down)'}"></i></span><span class="v">${v}</span></div>`).join('')||'<div class="empty">нет данных</div>';
 const byW={};sg.forEach(s=>{const k=s.window||'?';const o=byW[k]=byW[k]||{n:0,risk:0,fill:0,part:0,unf:0,rem:0,q:{}};o.n++;if(!s.risk_gate)o.risk++;if(s.signal_status==='EXECUTED')o.fill++;if(s.order_status==='PARTIAL')o.part++;if(s.signal_status==='ORDER_UNFILLED')o.unf++;o.rem+=s.remaining_seconds||0;o.q[s.execution_quality||'—']=(o.q[s.execution_quality||'—']||0)+1});
 $('ex_win').innerHTML=Object.entries(byW).sort().map(([k,v])=>`<tr><td><b>${esc(k)}</b></td><td>${v.n}</td><td>${v.risk}</td><td class="pos">${v.fill}</td><td>${v.part}</td><td class="${v.unf?'neg':''}">${v.unf}</td><td class="${v.fill/v.n>=.7?'pos':'warn'}">${(v.fill/v.n*100).toFixed(0)}%</td><td>${(v.rem/v.n).toFixed(0)} с</td><td>${(v.q.GOOD||0)} / ${(v.q.MARGINAL||0)} / ${(v.q.UNEXECUTABLE||0)}</td></tr>`).join('')||'<tr><td colspan="9" class="empty">нет сигналов</td></tr>';
 $('ex_list').innerHTML=sg.slice(-60).reverse().map(s=>`<tr><td>${(s.timestamp||'').slice(5,16).replace('T',' ')}</td><td>${esc(s.asset)}</td><td>${esc(s.window)}</td><td class="${s.direction==='UP'?'pos':'neg'}">${esc(s.direction)}</td><td>${fmt(s.entry_price)}</td>
  <td class="${s.signal_status==='EXECUTED'?'pos':'warn'}">${esc(s.signal_status)}</td><td class="${s.risk_gate?'neg':''}">${esc(s.risk_gate||'—')}</td><td>${esc(s.order_status||'—')}</td>
  <td class="${s.execution_quality==='GOOD'?'pos':s.execution_quality==='UNEXECUTABLE'?'neg':'warn'}">${esc(s.execution_quality||'—')}</td><td>${s.remaining_seconds==null?'—':Math.round(s.remaining_seconds)}</td></tr>`).join('')||'<tr><td colspan="10" class="empty">нет сигналов</td></tr>';
}
// ───── отчёты ─────
...
function showView(v){document.querySelectorAll('.view').forEach(x=>x.classList.toggle('on',x.id==='v_'+v));$('fabBack').style.display=v==='dash'?'none':'block';window.scrollTo(0,0);document.querySelectorAll('#menu .dd a').forEach(a=>a.classList.toggle('on',a.dataset.view===v));$('menu').classList.remove('open');
 if(v==='settings')loadSettings();if(v==='keys')loadKeys();if(v==='power')loadPower();if(v==='stats')loadArchive();if(v==='statistics')renderStats();if(v==='signals')renderSignals();if(v==='reports')loadReports();if(v==='execution')renderExec()}
// settings schema
const S=[
...
const VIEWS=['dash','settings','keys','power','stats','statistics','signals','reports','execution'];
